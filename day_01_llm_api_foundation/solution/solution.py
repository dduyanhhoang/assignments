...
def call_openai(
    ...
    logger.info(f"Starting call API")
    ...
    logger.info(f"Calling finished. {latency:.2f}")
...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        history = history[-6:]  # 3 turns = 6 messages (user+assistant pairs)
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<42} | {'GPT-4o':<42} | {'Mini':<42} | {'GPT-4o Latency':>14} | {'Mini Latency':>12}"
    sep = "-" * len(header)
    lines = [header, sep]
    for r in results:
        prompt = r.get("prompt", "")[:40]
        gpt4o = r.get("gpt4o_response", "")[:40]
        mini = r.get("mini_response", "")[:40]
        gpt4o_lat = f"{r.get('gpt4o_latency', 0):.3f}s"
        mini_lat = f"{r.get('mini_latency', 0):.3f}s"
        lines.append(f"{prompt:<42} | {gpt4o:<42} | {mini:<42} | {gpt4o_lat:>14} | {mini_lat:>12}")
    return "\n".join(lines)
